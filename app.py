...
def db():
    """Return a new DB session for this request."""
    return DBSession()
...
def save_lead(session_id: str, name: str, email: str, context: str = ''):
    s = db()
    # Avoid duplicate leads per session
...
def summarize_history(history: list[dict]) -> list[dict]:
    """Collapse the oldest messages into a single summary to save tokens."""
    if len(history) <= SUMMARIZE_AFTER:
        return history

    to_summarize = history[:-6]   # keep last 6 messages fresh
    recent       = history[-6:]
...
def extract_lead(text: str) -> tuple[str | None, str | None]:
    """Try to pull name + email out of a message."""
    email_match = re.search(r'[\w.+-]+@[\w-]+\.[a-z]{2,}', text, re.I)
    email = email_match.group(0) if email_match else None
    name  = None
    if email:
        # Look for "I'm <name>" or "my name is <name>" before the email
...
def api_chat():
    ...
    # Check for lead info in user's message
    name, email = extract_lead(user_msg)
    if email:
        save_lead(session_id, name or '', email, context=user_msg[:200])

    # Build message history (with optional summarization)
    history = get_history(session_id)
    # Remove the message we just added so we don't double it
    history = history[:-1]  # last item is the user msg we just stored
    history = summarize_history(history)
    history.append({'role': 'user', 'content': user_msg})

    # Call AI (Anthropic → Groq fallback)
    try:
        reply = chat_complete(
    ...
    # Also check the assistant reply for any email the user confirmed
...
def export_leads_csv():
    ...
    return Response(
        output.getvalue(),
        mimetype='text/csv',
        headers={'Content-Disposition': 'attachment; filename=harbor_leads.csv'}
    )
